...
class read_modfile:
	...
	def __init__(self,filelist):
		...
		self.struc=list(range(len(filelist)))
	...
	def read_single_file(self,modfile):
		...
		dep=[]
		dep.append(0.0)
		vp=list(range(nlayers))
		vs=list(range(nlayers))
		rho=list(range(nlayers))
		...
		self.deps = np.array(dep)
		self.rho = np.array(rho)
		self.alpha = np.array(vp)
		self.beta = np.array(vs)
		result=list(zip(dep,vs))
...
class read_egnfile:
    ...
    def __init__(self,flist,m_concerned,p_concerned):
        ...
        self.uz=list(range(len(flist)))
        self.ur=list(range(len(flist)))
        self.tz=list(range(len(flist)))
        self.tr=list(range(len(flist)))
        self.ut=list(range(len(flist)))
        self.tt=list(range(len(flist)))
    ...
    def parse_file(self,egn_file):
        ...
        if egn_file.endswith('.gz'):
            ...
            egn_contents = gzip.open(egn_file,'rt')
        else:
    ...
    def pick_right_slice(self):
        ...
        for line in rel_slice[1:]:
        	...
        	try:
        		...
        		islov = True
        if isray:
        	result[0]=list(zip(d,y1))
        	result[1]=list(zip(d,y2))
        	result[2]=list(zip(d,y3))
        	result[3]=list(zip(d,y4))
        	result[4]=None
        	result[5]=None
        elif islov:
        	result[4]=list(zip(d,y1))
        	result[5]=list(zip(d,y2))
        	result[0]=None
        	result[1]=None
        	result[2]=None
        	result[3]=None
...
class read_egnfile_per:
    ...
    def __init__(self,infile,p_concerned):
        if infile.endswith('.gz'):
            ...
            egn_contents = gzip.open(infile,'rt')
        else:
        ...
        try:
            ...
            raise Exception("Error reading input file %s" %(infile))
        ...
        alpha=list(range(model_deps))
        beta=list(range(model_deps))
        ...
        for line in egn_contents:
        	...
        	if (len(line.split())==ncol_ph) and (float(line.split()[0]).is_integer()):
        		format_match=True
        		per_rounded=round(float(line.split()[1]),4)
        		if per_rounded==round(p_concerned,4):
        			if __name__=='__main__':
        ...
        if format_match:
        	try:
        		...
        		raise Exception("Cannot find period sample: %f s in eigenfunction file" %(p_concerned))
        else:
        	raise Exception("Error reading input file %s" %(infile))
...
class read_disp:
    ...
    def __init__(self,flist,int1,int2):
        ...
        if int1==None and int2==None:
            ...
            self.getfmod=True
            self.fstruc=list(range(len(flist)))
            for fn,fl in enumerate(flist):
            ...
            self.mnum=int1
            self.psec=int2
            self.modpd=list(range(len(flist)))
            for fn,fl in enumerate(flist):
            ...
            self.mode_l = int1
            self.mode_h = int2
            self.modcdisp=list(range(len(flist)))
            self.modudisp=list(range(len(flist)))
            for fn,fl in enumerate(flist):
            	try:
    ...
    def read_single_file(self,indfile):
    	...
    	if self.getpd:
    		...
    		return list(zip(self.deporig,self.reqdpd))
    	elif self.getdisp:
    		...
    		return list(zip(self.depf,self.vsf))
    ...
    def parse_filepd(self,disp_file,modonly=False):
        ...
        if disp_file.endswith('.gz'):
            ...
            disp_contents = gzip.open(disp_file,'rt')
        else:
                ...
                if foundtarget:
                    ...
                    if (len(line.split())==self.ncol_ph): #and ("mode" not in line.split()):
                        ...
                        try:
                        	...
                        	sys.exit()
                        if (int(line.split()[0])==self.mnum) and (per_rounded==self.psec):
                        	foundtarget=True
    ...
    def pick_right_slice(self):
    	...
    	pvd=list(range(len(self.rel_modes))) # pvd stands for phase velocity dispersion
    	gvd=list(range(len(self.rel_modes))) # gvd stands for group velocity dispersion
    	pv=[]
    	gv=[]
    	freq=[]
    	mdone=0
    	for line in rel_slice:
    		if "****" in line:
    			try:
    				pvd[mdone]=list(zip(freq,pv))
    			except IndexError:
    				break
    			gvd[mdone]=list(zip(freq,gv))
    			if __name__ == '__main__':
    				print("Finished reading dispersion of mode %d" %(self.rel_modes[mdone]))
    			mdone+=1
    			pv=[]
    			gv=[]
    			freq=[]
...
class read_excitation:
    ...
    def __init__(self,exfile,freq_interest):
        ...
        if exfile.endswith('.gz'):
            ...
            ex_contents = gzip.open(exfile,'rt')
        else:
